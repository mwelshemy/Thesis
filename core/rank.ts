...
export function rankCandidates(snippets: CodeSnippet[], query?: string): CodeSnippet[] {
  ...
  const scored = snippets.map(snippet => {
    ...
    for (const v of variants) {
      if (fileNameNoExt === v.toLowerCase()) { score = Math.max(score, 0.9); reasons.push('Filename exact'); }
      else if (fileNameNoExt.includes(v.toLowerCase())) { score += 0.6; reasons.push('Filename partial'); }
    }
    if (snippet.symbolSignature) {
      for (const v of variants) {
        if ((snippet.symbolSignature || '').toLowerCase().includes(v.toLowerCase())) { score += 0.8; reasons.push('Signature contains token'); }
      }
    }
    for (const v of variants) {
      if (contentLower.includes(v.toLowerCase())) { score += 0.25; reasons.push('Token in content'); }
    }
