...
// ✅ Load .env properly (works after compile)
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand("mockui.showMockUI", () => {
    ...
    if (!fs.existsSync(htmlPath)) {
      vscode.window.showErrorMessage("❌ webview/index.html not found!");
      return;
    }
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
      if (msg.command === "sendPrompt") {
        ...
        try {
          ...
          panel.webview.postMessage({ command: "aiResponse", text: `❌ ${err.message}` });
        }
      }
    });
  });
