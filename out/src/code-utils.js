...
function extractCodeSections(content, language) {
    ...
    try {
        ...
        if (['javascript', 'typescript'].includes(language)) {
            const functionRegex = /(?:export\s+default\s+|export\s+)?(?:function\s+([A-Za-z_$][\w$]*)\s*\(|(?:const|let|var)\s+([A-Za-z_$][\w$]*)(?:\s*:\s*[^=]+)?\s*=\s*(?:<[^>]+>\s*)?(?:async\s*)?(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>|class\s+([A-Za-z_$][\w$]*))/g;
            let match;
        ...
        if (language === 'python') {
            ...
            while ((match = functionRegex.exec(content)) !== null) {
                ...
                const codeBlock = extractCodeBlock(content, match.index, language);
                if (codeBlock)
                    sections.push({ name: match[1], code: codeBlock, type: 'function', lineNumber: startLine });
            }
